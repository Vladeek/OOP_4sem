        ...
        private void button_send_Click(object sender, EventArgs e)// уже лучше  
        {
            if (ValidateForm())
            {
                ...
                foreach (RadioButton el in groupBox_spec.Controls)
                {
                    if (el.Checked)
                    {
                        if (el.Name.Contains("ПОИТ")) newStudent.Spec = Student.SPEC.POIT;
                        if (el.Name.Contains("ИСИТ")) newStudent.Spec = Student.SPEC.ISIT;
                        if (el.Name.Contains("ПОИБМС")) newStudent.Spec = Student.SPEC.MOBILE;
                        if (el.Name.Contains("ДИЗАЙНЕР")) newStudent.Spec = Student.SPEC.DESIGNER;
                    }
                }
                newStudent.Birthday = dateTimePicker_birthday.Value;
                foreach (RadioButton el in groupBox_course.Controls)
                {
                    if (el.Checked) newStudent.Course = byte.Parse(el.Name.Reverse().First().ToString());
                }
